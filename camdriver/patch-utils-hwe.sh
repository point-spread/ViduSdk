...
function choose_kernel_branch {
	...
	then
		...
		*)
			#error message shall be redirected to stderr to be printed properly
			echo -e "\e[31mUnsupported kernel version $1 . The Focal patches are maintained for Ubuntu LTS with kernel 5.15, 6.2 only\e[0m" >&2
			exit 0
			;;
		esac

	else
		echo -e "\e[31mUnsupported distribution $2, kernel version $1 . The patches are maintained for Ubuntu 16/18/20/22 LTS\e[0m" >&2
		exit 1
	fi
}
